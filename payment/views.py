...
stripe_endpoint_key = settings.STRIPE_WEBHOOK_SECRET
    ...
    if session_id:
        try:
            ...
            if request.headers.get("HX-Request"):
                return TemplateResponse(request,'payment/stripe_success_content.html', {'context': context})
...
def stripe_cancel(request):
    ...
    if order_id:
        ...
        context = {'order': order}
        if request.headers.get("HX-Request"):
            return TemplateResponse(request, 'payment/stripe_cancel_content.html', {'context': context})
